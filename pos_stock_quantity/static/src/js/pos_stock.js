...
odoo.define('pos_stock_quantity.pos_stock', function (require) {
    ...
    models.PosModel = models.PosModel.extend({
        ...
        load_qty_after_load_product: function () {
            var wait = this.get_model('pos.payment.method');
            var _wait_super_loaded = wait.loaded;
            wait.loaded = function (self, payment_methods) {
                var done = $.Deferred();
                _wait_super_loaded(self, payment_methods);
        ...
        stock_location_modifier: function () {
            ...
            var stock_location = this.get_model('product.product');
            var _super_loaded = stock_location.loaded;

            stock_location.loaded = function (self, products) {
        ...
        refresh_qty: function () {
			var self = this;
            $('.product-list-container,.product-list-contents').find('.qty-tag').each(function () {
                var $product = $(this).parents('.product-list-row');
                if($product.length==0){
                    $product = $(this).parents('tr');
                }
    ...
    /****************************************************
     *  TO FIX
     *  Entra en conflicto con pos_lan y pos_multi_uom_price
     * 
    ...
    models.Orderline = models.Orderline.extend({
        ...
        check_reminder: function () {
            ...
            if (qty_available - sum_qty < this.pos.config.limit_qty) {
				...
				if (qty_available - sum_qty + self.quantity - this.pos.config.limit_qty < 0){
					max_available = 0;
				}
