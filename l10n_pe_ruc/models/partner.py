...
class ResPartner(models.Model):
            ...
            if values and values.get('document_type_sunat_id'):
                ...
                if document_type_code == '6':
                    ...
                    if len(ubigeo)==6:
                        ...
                        values['l10n_pe_district'] = district_id
                if partner_id:
    ...
    def _onchange_l10n_pe_district(self):
        if not self.l10n_pe_district:
            self.zip = ''
            return
        self.zip = self.l10n_pe_district.code

    @api.onchange('vat','zip')
    def _onchange_vat_search_peru(self):
        if not self.l10n_latam_identification_type_id or not self.vat:
            return
        partner_id = self.id
        vat = self.vat
        document_type = self.l10n_latam_identification_type_id.id
        token_api = self.env.company.token_api_ruc
        if document_type and vat and token_api:
            document_type_code = self.env['l10n_latam.identification.type'].browse(document_type).l10n_pe_vat_code
            obj_sunat_conflux = SunatPartnerConflux(vat, document_type_code, token_api)
            ...
            if values and values.get('document_type_sunat_id'):
                ...
                self.company_type = values['company_type']
                self.document_type_sunat_id = obj_document_type_origin
                self.l10n_latam_identification_type_id = obj_document_type_origin
                if document_type_code == '6':
